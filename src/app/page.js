...
function ChurchMembershipSystem() {
  ...
  const getMeetingDates = (dayOfWeek, refDate = null) => {
    ...
    // Ajustar para o dia da semana alvo mais prÃ³ximo (pode ser futuro se for amanhÃ£)
    // Se hoje Ã© SÃ¡bado (6) e alvo Ã© Domingo (0), e queremos ver o futuro:
  ...
  const addVisitor = async (e) => {
    ...
    if (error) {
      ...
      alert('Visitante registrado com sucesso! O formulÃ¡rio foi limpo para o prÃ³ximo.');
    }
  };
      ...
      <main className="flex-1 overflow-auto flex flex-col relative">
        ...
        <div className="p-6 md:p-10 space-y-6 md:space-y-10 animate-in fade-in duration-500 max-w-[1600px] mx-auto w-full">
          ...
          {isLeaderMode && activeTab === 'leader-attendance' && (
            <div className="space-y-6">
              <header className="flex justify-between items-center"><h2 className="text-2xl font-black italic uppercase tracking-tighter">HistÃ³rico de FrequÃªncia</h2></header>
              <div className={`${t.card} border rounded-2xl overflow-x-auto`}>
                <table className="w-full text-left">
                  <thead className={`${t.tableHead} border-b ${t.border}`}>
                    <tr>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar-fine">
                ...
                <div className={`p-4 rounded-2xl border flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 ${getVisitorConsecutiveAttendances(memberForm) >= 6 ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' : 'bg-pink-500/5 border-pink-500/10 text-pink-400'}`}>
                  <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-xl flex flex-col items-center justify-center font-black leading-tight ${getVisitorConsecutiveAttendances(memberForm) >= 6 ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/30' : 'bg-pink-500/20 text-pink-400'}`}>
                      ...
                      <span className="text-[6px] uppercase tracking-tighter">CÃ©lulas</span>
                    </div>
                    <div>
                      ...
                      <p className="text-[9px] text-slate-400 font-bold">
                        {getVisitorConsecutiveAttendances(memberForm) >= 6 
                          ? 'âœ¨ Atingiu a meta de 6+ presenÃ§as! Clique para convertÃª-lo em membro.' 
                          : `Faltam ${Math.max(0, 6 - getVisitorConsecutiveAttendances(memberForm))} presenÃ§as seguidas na cÃ©lula para se tornar membro.`}
                      </p>
                    </div>
                  </div>
                  {getVisitorConsecutiveAttendances(memberForm) >= 6 && (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-6">
                  ...
                  {!isLeaderMode && (<div className={`${darkMode ? 'bg-white/5' : 'bg-slate-50'} p-3 rounded-xl border ${t.border}`}><p className="text-[8px] font-black text-slate-500 uppercase mb-2">CÃ‰LULA</p><select value={memberForm.cell_id} onChange={e => setMemberForm({ ...memberForm, cell_id: e.target.value })} className="w-full bg-transparent font-black text-sm outline-none"><option value="">Selecionar...</option>{cells.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>)}
                  <div className="space-y-2">
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-[100] flex items-center justify-center p-2 sm:p-4 overflow-hidden">
          <div className={`${darkMode ? 'bg-[#0f172a]' : 'bg-white'} w-full max-w-xl max-h-[95vh] rounded-2xl border ${t.border} shadow-2xl flex flex-col relative`}>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar-fine text-left">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className={`${darkMode ? 'bg-white/5' : 'bg-slate-50'} p-3 rounded-xl border ${t.border}`}><p className="text-[8px] font-black text-slate-500 uppercase mb-2">DIA DA SEMANA</p><select value={cellForm.day_of_week} onChange={e => setCellForm({ ...cellForm, day_of_week: e.target.value })} className="w-full bg-transparent font-black text-sm outline-none"><option value="segunda">Segunda</option><option value="terÃ§a">TerÃ§a</option><option value="quarta">Quarta</option><option value="quinta">Quinta</option><option value="sexta">Sexta</option><option value="sÃ¡bado">SÃ¡bado</option><option value="domingo">Domingo</option></select></div>
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
        <div className="bg-emerald-500/10 border border-emerald-500/20 p-6 rounded-3xl">
          ...
          <p className="text-3xl font-black text-emerald-500 italic">{members.filter(m => m.attended_cell).length} <span className="text-sm opacity-50">Membros na CÃ©lula</span></p>
        </div>
