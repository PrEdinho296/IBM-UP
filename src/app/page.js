...
function ChurchMembershipSystem() {
  ...
  const [memberTypeFilter, setMemberTypeFilter] = useState('membros'); // 'membros', 'todos', 'oficiais', 'visitantes', 'consolidacao'
  ...
  const filteredMembers = members.filter(m => {
    ...
    if (memberTypeFilter === 'membros') matchesType = !m.outros;
    else if (memberTypeFilter === 'oficiais') matchesType = m.membro_igreja;
          ...
          <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-md z-[9999] flex items-center justify-center p-4 overflow-y-auto">
            <form onSubmit={saveLeaderConfig} className={`${darkMode ? 'bg-[#0f172a]' : 'bg-white'} w-full max-w-md rounded-3xl border ${t.border} shadow-2xl flex flex-col relative text-left my-auto`}>
              ...
              <div className="p-6 space-y-4">
                ...
                {/* Seletor de célula â€” mostra todas as células sem login configurado */}
                {!activeCell && (
                  <div>
                    ...
                    {selectedCellForConfig?.login_email && (
                      <p className="text-[9px] text-amber-400 font-bold uppercase mt-2 flex items-center gap-1">
                        âš  Esta célula já tem acesso configurado. Ao salvar, as credenciais serão substituídas.
                      </p>
                    )}
                  </div>
                )}
          ...
          {(activeTab === 'members' || activeTab === 'leader-members') && (
            <div className="space-y-6">
              ...
              <div className="flex bg-white/5 p-1 rounded-xl border border-white/5 self-start mb-6 inline-flex flex-wrap gap-1">
                {['membros', 'oficiais', 'visitantes', 'consolidacao', 'todos'].map(f => (
                  <button 
                    key={f} 
                  ...
                  >
                    {f === 'membros' ? 'Membros da Igreja' : f === 'todos' ? 'Ver Todos' : f === 'oficiais' ? 'Membros Oficiais' : f === 'visitantes' ? 'Visitantes' : 'Em Consolidação'}
                  </button>
                ))}
              </div>
              ...
              <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                  <div>
                    ...
                    <p className="text-[10px] text-blue-500 font-black uppercase tracking-widest mt-1">Visão 360Âº de presença nas últimas 15 semanas</p>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="text-[9px] text-slate-400 font-bold">
                      {getVisitorConsecutiveAttendances(memberForm) >= 6 
                        ? 'ðŸš€ Atingiu a meta de 6+ presenças! Clique para convertê-lo em membro da célula.' 
                        : `Faltam ${Math.max(0, 6 - getVisitorConsecutiveAttendances(memberForm))} presenças seguidas para se tornar membro da célula.`}
                    </p>
                  </div>
                  <button 
                      type="button"
