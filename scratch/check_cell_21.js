...
async function checkCell21() {
  const { data: cell, error } = await supabase.from('cells').select('*').eq('id', 21).single();
  if (error) {
    console.error('Error fetching cell 21:', error);
  } else {
    console.log('Cell 21:', cell);
  }
}
